...
def test_ramazan_matches_the_observed_collapse(year: int, expected: date) -> None:
    ...
    assert _first("ramazan", year) == expected
...
def test_kurban_matches_the_observed_collapse(year: int, expected: date) -> None:
    assert _first("kurban", year) == expected
...
def test_religious_holidays_slide_earlier_each_year() -> None:
    ...
    day_of_year = [_first("ramazan", y).timetuple().tm_yday for y in (2023, 2024, 2025, 2026)]
...
def test_the_two_feasts_keep_a_constant_gap() -> None:
    gaps = [(_first("kurban", y) - _first("ramazan", y)).days for y in (2024, 2025, 2026)]
...
def test_an_administrative_extension_is_honoured() -> None:
    ramazan = next(h for h in religious_holidays(2024) if h.name == "ramazan")

    # 2024 was extended to a nine-day break by government decision — something no
    # calendar can compute, which is why it is recorded by hand.
    assert ramazan.length_days == 6
    assert ramazan.last_day == date(2024, 4, 15)
